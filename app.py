...
def waterfall_chart(df, x_col, y_col, title="Waterfall"):
    d = df.groupby(x_col)[y_col].sum().reset_index().head(15)
    colors = ["#6fcf97" if v >= 0 else "#eb5757" for v in d[y_col]]
...
def funnel_chart(df, x_col, y_col, title="Funnel"):
    d = df.groupby(x_col)[y_col].sum().reset_index().sort_values(y_col, ascending=False).head(8)
...
with tab_overview:
    ...
    if cat_cols and num_cols:
        with c1:
            fig = bar_chart(
                df.groupby(cat_cols[0])[num_cols[0]].sum().reset_index().sort_values(num_cols[0], ascending=False).head(12),
        ...
        with c2:
            fig = pie_donut(
                df.groupby(cat_cols[0])[num_cols[0]].sum().reset_index().head(8),
                cat_cols[0], num_cols[0],
    ...
    if date_cols and num_cols:
        fig = area_chart(
            df.groupby(date_cols[0])[num_cols[0]].sum().reset_index(),
            date_cols[0], num_cols[0],
...
with tab_charts:
    ...
    if chart_type in ["Bar Chart", "Grouped Bar", "Line Chart", "Area Chart",
        ...
        agg_df = df.groupby([x_col] + ([color_col] if color_col else []))[y_col].sum().reset_index()
        ...
        agg_df = df.groupby(names_col)[vals_col].sum().reset_index().head(12)
...
with tab_advanced:
    ...
    with c2:
        ...
        if cat_cols and num_cols:
            ...
            if path_sel:
                agg_tm = df.groupby(path_sel)[val_sel].sum().reset_index()
